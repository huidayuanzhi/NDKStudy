#include "Java_com_cheng_ndkstudy_JniClient.h"
#include <stdlib.h>
#include <stdio.h>
#include "string.h"
// 引入log头文件
...
JNIEXPORT void JNICALL Java_com_cheng_ndkstudy_JniClient_callJavaStaticMethod(
    ...
    int i = 0;
    mJclass = (*env)->FindClass(env, "com/cheng/ndkstudy/MyJavaClass"); //JNIEnv*, const char*
    if (mJclass == NULL) {
    ...
    mJStaticmethodID = (*env)->GetStaticMethodID(env, mJclass,
                                                 "callStaticMethod",
                                                 "(Ljava/lang/String;I)V"); //JNIEnv*, jclass, const char*, const char*
...
JNIEXPORT void JNICALL Java_com_cheng_ndkstudy_JniClient_callJavaInstaceMethod(
    ...
    // 1、从classpath路径下搜索ClassMethod这个类，并返回该类的Class对象
    mJclass = (*env)->FindClass(env, "com/cheng/ndkstudy/MyJavaClass");
    if (mJclass == NULL) {
    ...
    jobjectMyClass = (*env)->NewObject(env, mJclass,
                                       jmethodID_Construct); //JNIEnv*, jclass, jmethodID, ...
...
//
//	// 1、获取Cat类的class引用
//	cls_cat = (*env)->FindClass(env, "com/cheng/ndkstudy/Cat");
//	if (cls_cat == NULL) {
//		return;
//	}
...
//	cls_animal = (*env)->FindClass(env, "com/cheng/ndkstudy/Animal");
//	if (cls_animal == NULL) {
//		return;
//	}
//
...
Java_com_cheng_ndkstudy_JniClient_getObjectFromC(JNIEnv *env, jobject obj, jstring jstr) {
    ...
    mid_cat_init = (*env)->GetMethodID(env, cls_cat, "<init>",
                                       "(Ljava/lang/String;)V");
    if (mid_cat_init == NULL) {
        return NULL; // 没有找到只有一个参数为String的构造方法
    }

    // 3、创建一个String对象，作为构造方法的参数
    c_str_name = (*env)->NewStringUTF(env, "Tom Cat");
    if (c_str_name == NULL) {
        return NULL; // 创建字符串失败（内存不够）
    }
